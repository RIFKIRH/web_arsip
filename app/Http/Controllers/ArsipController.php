...
class ArsipController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($arsip)
            ...
            // Kolom kurun_waktu (gabungan tanggal mulai dan selesai)
            ...
            // Filter search hanya pada kolom yang tampil di tabel
            ->filter(function ($query) use ($request) {
                if (!empty($request->search['value'])) {
                    $search = $request->search['value'];
                    $query->where(function ($q) use ($search) {
                        $q->where('url_file', 'like', "%{$search}%")
                          ->orWhere('jumlah', 'like', "%{$search}%")
                          ->orWhere('tingkat_perkembangan', 'like', "%{$search}%")
                          ->orWhere('keterangan_nomor_box', 'like', "%{$search}%")
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'file_upload' => 'nullable|file|max:2048|required_without:url_file',
            'url_file' => 'nullable|required_without:file_upload',
